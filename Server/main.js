...
MongoClient.connect(url, function (err, database) {
    null === err ? console.log("Connected to MongoDB server") : console.log(err);
    db.users = database.collection('users');
    db.rooms = database.collection('rooms');
    db.votings = database.collection('votings');
    //db.rooms_users = database.collection('rooms_users');
...
io.on(CONNECTION, function (socket) {
    ...
    socket.on(PREVIEW_ROOM, function (msg) {
        var roomsDetails;
        if (undefined != msg.id) {
            roomsDetails = findRoomById(msg.id);
        } else if (undefined != msg.name && undefined != msg.password) {
            roomsDetails = findRoomByNameAndPass(msg.name, msg.password);
        }

        if (undefined != roomsDetails && false != roomsDetails) {
            previewRoom(roomsDetails);
        } else {
            ...
            db.rooms.replaceOne(
                ...
                function (err, res) {
                    if (null === err) {
                        ...
                        // check admin status
                        designateAdmin(roomsDetailsToSave);
...
function designateAdmin(roomsDetails) {
